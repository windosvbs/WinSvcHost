...
def beacon_loop():
    """Anuncia la presencia del servidor en la red local cada 3 segundos"""
    try:
        ...
        while True:
            ...
            try:
                if   action == "mouse_move"   and PYAUTOGUI_OK: pyautogui.moveTo(cmd["x"], cmd["y"])
                    ...
                    try: pyautogui.mouseDown(cmd["x"], cmd["y"], button=cmd.get("button","left"))
                    except: pass
                elif action == "mouse_up" and PYAUTOGUI_OK:
                    try: pyautogui.mouseUp(cmd["x"], cmd["y"], button=cmd.get("button","left"))
                    except: pass
                elif action == "type_text" and PYAUTOGUI_OK:
                    try: pyautogui.typewrite(cmd.get("text",""), interval=0)
                    except: pass
                        ...
                        for d in string.ascii_uppercase:
                            ...
                            if os.path.exists(p):
                                try:
                                    ...
                                    size = 0
                                roots.append({"name": p, "is_dir": True, "size": size, "modified": 0})
                    else:
                        home = os.path.expanduser("~")
                        roots = [
                            {"name": "/",    "is_dir": True, "size": 0, "modified": 0},
                            {"name": home,   "is_dir": True, "size": 0, "modified": 0},
                        ]
                    ...
                    try:
                        ...
                        if size > 500 * 1024 * 1024:
                            res["result"] = json.dumps({"error": "Archivo muy grande (>500MB)"})
                        else:
...
def handle_screen(conn, addr):
    try:
        ...
        prev_hash   = None
        monitor_box = None
        quality     = 75   # calidad JPEG por defecto
        scale       = 1.0  # sin reducción por defecto
        ...
        while True:
            try:
                ...
                if req_s.startswith("MONITOR:"):
                    try:
                        ...
                        monitor_box = None
                    continue
                if req_s.startswith("QUALITY:"):
                    try: quality = int(req_s[8:])
                    except: pass
                    continue
            ...
            if PIL_OK:
                try:
                    ...
                    buf = io.BytesIO()
                    s.save(buf, format="JPEG", quality=quality,
                           optimize=False, subsampling=2)
                    d = buf.getvalue()

                    # Solo enviar si cambió la imagen
                    h = hash(d)
                    if h == prev_hash:
