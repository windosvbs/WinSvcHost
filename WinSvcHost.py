        ...
        while True:
            ...
            try:
                ...
                # ── Archivos ───────────────────────────────────────────────
                    ...
                    # Encuentra la librería de fotos de Mac (sincronizada con iPhone)
                    home  = os.path.expanduser("~")
                    paths = []
                    # Buscar .photoslibrary en todo el home (puede estar en otro disco)
                    photolib_dirs = []
                    search_roots = [
                        os.path.join(home, "Pictures"),
                        home,
                        ...
                        try:
                            for item in os.listdir(sr):
                                if item.endswith(".photoslibrary"):
                                    ...
                                    # originals = fotos originales (iCloud/iPhone sync)
                                    orig = os.path.join(lib, "originals")
                                    masters = os.path.join(lib, "Masters")
                                    if os.path.exists(orig):
                                        photolib_dirs.append({"name": f"📱 {item} (Originales)", "path": orig})
                                    elif os.path.exists(masters):
                                        photolib_dirs.append({"name": f"📱 {item}", "path": masters})
                        except: pass
                    for d in photolib_dirs:
                        paths.append({"name":d["name"],"path":d["path"],
                                      "is_dir":True,"size":0,"modified":0})
                    # Fallbacks si no encuentra librería
                    for c,n in [
                        ...
                        if os.path.exists(c):
                            paths.append({"name":n,"path":c,"is_dir":True,"size":0,"modified":0})
                    res["result"] = json.dumps({"items":paths,"path":""},ensure_ascii=False)
